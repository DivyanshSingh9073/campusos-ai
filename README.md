...
# db:setup runs `psql $DATABASE_URL -f src/db/schema.sql`, and psql doesn't
# read .env files — export it in your shell first:
export DATABASE_URL=postgresql://postgres:postgres@localhost:5432/campusos_ai
npm run db:setup
```
...
Documented honestly rather than hidden — these are intentional scope boundaries, not bugs:
